...
def spid_logout(request, config_loader_path=None, **kwargs):
    ...
    slo = client.metadata.single_logout_service(subject_id.name_qualifier, binding, "idpsso")

    if len(slo) > 1:
        logger.warning(f"spid_logout => more than 1 logout binding")

    slo_location = slo[0]['location']
    if not slo_location:
...
def spid_sp_metadata(conf: 'saml2.config.SPConfig'):
    """
    ...
    # for (cnt, attribute_consuming_service) in enumerate(_spsso_descriptor.attribute_consuming_service): # type: int, AttributeConsumingService
    #    attribute_consuming_service.index = str(cnt)
    #    for reqattr in attribute_consuming_service.requested_attribute:
