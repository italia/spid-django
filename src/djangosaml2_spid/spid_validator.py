        ...
        for i in self.response.assertion:
            for subject_confirmation in i.subject.subject_confirmation:
                ...
                if not subject_confirmation.subject_confirmation_data.in_response_to:
                    raise SpidError('subject.subject_confirmation_data in response -> null data')
                ...
                if self.recipient != subject_confirmation.subject_confirmation_data.recipient:
                    ...
                    raise SpidError(msg.format(subject_confirmation.subject_confirmation_data.recipient))
                ...
                if not hasattr(subject_confirmation.subject_confirmation_data, 'not_on_or_after') or \
                    ...
                    raise SpidError('subject.subject_confirmation_data not_on_or_after not valid')
        ...
        for i in self.response.assertion:
            ...
            for aud in i.conditions.audience_restriction:
                if not getattr(aud, 'audience', None):
                    raise SpidError(('Assertion conditions audience_restriction '
                                     'without audience'))
        ...
        for i in self.response.assertion:
            ...
            for authns in i.authn_statement:
                ...
                if authns.authn_context.authn_context_class_ref.text \
                    ...
                    raise SpidError('Assertion authn_statement.authn_context.'
                ...
                for attri in i.attribute_statement:
                    if not attri.attribute:
                        raise SpidError('Assertion attribute_statement.attribute is missing/invalid')
